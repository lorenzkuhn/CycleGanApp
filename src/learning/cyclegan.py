...
def train():
    ...
    for epoch_index in range(n_epochs):
        for _ in range(n_discriminator_steps):
            ...
            cycle_data_updated = utils.CycleData(discr_x, discr_y, gen_xy, gen_yx, cycle_data.real_x, cycle_data.real_y)
        ...
        l1norm = nn.L1Loss(reduction='mean').to(device)
        cyclic_consistency_x = l1norm(cycle_data_updated.id_x_approximations, data.real_x)
        cyclic_consistency_y = l1norm(cycle_data_updated.id_y_approximations, data.real_y)
        print("Epoch {} Cyclic consistency loss X: {}".format(epoch_index, cyclic_consistency_x))
        print("Epoch {} Cyclic consistency loss Y: {}\n".format(epoch_index, cyclic_consistency_y))
